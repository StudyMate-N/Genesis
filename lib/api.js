/**
 * ═══════════════════════════════════════════════════
 * VERITAS ACADEMY ACADEMIC — Unified API Service Layer
 * 
...
async function sendEmail({ to, subject, html, from }) {
  try {
    ...
    const fromAddr = from || process.env.EMAIL_FROM || 'Veritas Academy <noreply@veritas.academy>';
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.06)">
  <tr><td style="background:${headerBg};padding:32px 40px;text-align:center">
    <div style="display:inline-block;background:#ecc94b;border-radius:10px;padding:8px 12px;margin-bottom:16px">
      <span style="font-size:18px;font-weight:800;color:#0f172a;letter-spacing:1px">VERITAS ACADEMY</span>
    </div>
    ${headerContent}
  </td></tr>
  ...
  <tr><td style="background:#f8fafc;border-top:1px solid #e2e8f0;padding:24px 40px;text-align:center">
    <p style="margin:0;font-size:12px;color:#94a3b8">Veritas Academy &middot; <a href="${siteUrl}" style="color:#64748b">veritas.academy</a></p>
  </td></tr>
...
const api = {
  ...
  sendQuoteEmail: async (order) => {
    ...
    const result = await sendEmail({
      ...
      subject: `Your Veritas Academy Study Plan — $${order.quoteAmount}`,
      html,
    });
